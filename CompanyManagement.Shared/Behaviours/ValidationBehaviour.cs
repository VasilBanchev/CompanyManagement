    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        if (failuresAsync.Count != 0)
        {
            ...
            if (responseType.IsGenericType)
            {
                ...
                var invalidResponseType = typeof(ModelWrapper<>).MakeGenericType(resultType);
            ...
            return (Activator.CreateInstance(typeof(ModelWrapper), "Invalid state of the data", failuresAsync.Select(s => s.ErrorMessage).ToList()) as TResponse)!;
        }
